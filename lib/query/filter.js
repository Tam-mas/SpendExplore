 ...
 *
 * Excluded rows, income rows and transfers are omitted unless explicitly
 * requested — they would otherwise silently distort every spend total.
 *
 * Transfers are money moving between the household's OWN accounts, not a
 * spending decision, so counting them answers no question this app asks. On a
 * real ledger they were 28% of "total spend", which also made every
 * period-comparison delta swing on how much money got shuffled that month
 * rather than on how much was spent.
 */
export function applyFilters(transactions, filters = {}, ctx) {
  const {
    ...
    includeTransfers = false, excludeIds
  } = filters;
  ...
  return transactions.filter((txn) => {
    ...
    if (!includeTransfers && txn.categoryId === 'transfers') return false;
