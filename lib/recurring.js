...
import { shiftMonth, daysInMonth, addYears } from './query/periods.js';
...
// Weekly and fortnightly are genuinely day-based cadences, so addDays is
// correct for them. Monthly/quarterly/annual are calendar cadences — a bill
// due "on the 15th" is due on the 15th of the next calendar month, not 30.44
// days later, and a naive round(30.44) = 30 makes every monthly nextExpected
// one day early forever (worse for quarterly: two days early AND in the
// wrong month). lib/query/periods.js already does this calendar arithmetic
// correctly (UTC-based, day-of-month clamped) for period-comparison windows,
// so this reuses it directly rather than re-deriving the same month/leap-year
// handling a second time in this file.
const addCalendarMonths = (iso, n) => {
...
const NEXT_EXPECTED = {
  weekly: (iso, cadence) => addDays(iso, cadence.days),
  fortnightly: (iso, cadence) => addDays(iso, cadence.days),
  monthly: (iso) => addCalendarMonths(iso, 1),
  quarterly: (iso) => addCalendarMonths(iso, 3),
  annual: (iso) => addYears(iso, 1)
};

/**
 * Sum committed totals for a group of series, rounding the pooled annual
 * figure ONCE rather than rounding each series's monthly share and summing
 * the rounded pieces. The latter compounds up to half a cent of loss per
 * series, which becomes a visible (and confusing) `committedMonthly * 12 !==
 * committedAnnual` once more than one or two series are committed.
 */
const sumCommitted = (list) => {
  const rawAnnual = list.reduce((a, s) => a + s.annualCost, 0);
  return { monthly: round2(rawAnnual / 12), annual: round2(rawAnnual) };
};

/**
 * The four committed-spend totals derived from one series list: the
 * high-confidence headline (what the user can trust literally) and the
 * medium-confidence "possibly recurring" subtotal (still detected and
 * listed, just not folded into a number presented as a commitment). Shared
 * by detectRecurring and applyOverrides so the two can never compute the
 * rule differently.
 */
function committedTotals(series) {
  const active = series.filter((s) => s.status === 'active');
  const high = sumCommitted(active.filter((s) => s.confidence === 'high'));
  const medium = sumCommitted(active.filter((s) => s.confidence === 'medium'));
  return {
    committedMonthly: high.monthly,
    committedAnnual: high.annual,
    uncertainMonthly: medium.monthly,
    uncertainAnnual: medium.annual
  };
}
...
export function detectRecurring(snapshot, { today = new Date().toISOString().slice(0, 10), strict = true } = {}) {
  ...
  for (const [merchant, group] of byMerchant) {
    ...
    const nextExpected = NEXT_EXPECTED[cadence.cadence](lastDate, cadence);
  ...
  // A medium-confidence series (thin history, a skip, or a variable amount)
  // is still listed but kept OUT of the headline for the same reason —
  // summing a coincidence into "committed" is the exact failure this module
  // exists to avoid.
  return { series, ...committedTotals(series) };
}

export const OVERRIDE_DECISIONS = Object.freeze(['recurring', 'ignored', 'auto']);

/**
 * A forced-recurring series for a merchant whose gaps don't fit ANY cadence
 * band — a real bill that just doesn't land on a clean schedule (paid on
 * different days each month, say). detectCadence rightly refuses to call
 * this a "monthly" or "fortnightly" charge, but the user has explicitly said
 * it recurs, so it still needs to show up rather than silently vanish. Cost
 * is annualised off the AVERAGE gap between charges instead of a named
 * cadence — an honest estimate rather than a false precision.
 *
 * Returns null only when there isn't enough history to estimate anything
 * (zero transactions) — a single occurrence still renders, just with no
 * annualised cost, because "not enough data yet" is a fact worth showing,
 * not a reason to hide the row.
 */
function buildManualSeries(merchant, ordered) {
  if (!ordered.length) return null;

  const profile = amountProfile(ordered.map((t) => t.amount));
  const firstDate = ordered[0].date;
  const lastDate = ordered.at(-1).date;
  const spanDays = ordered.length > 1 ? (toUTC(lastDate) - toUTC(firstDate)) / MS_PER_DAY : 0;
  const perYear = spanDays > 0 ? 365.25 / (spanDays / (ordered.length - 1)) : 0;
  const annualCost = round2(profile.typical * perYear);
  const monthlyCost = round2(annualCost / 12);
  ...
  return {
    ...
    categoryId: ordered.at(-1).categoryId,
    cadence: 'irregular',
    cadenceLabel: ordered.length > 1 ? 'Irregular' : 'Not enough data yet',
    cadenceDays: spanDays > 0 ? spanDays / (ordered.length - 1) : null,
    confidence: 'medium',
    occurrences: ordered.length,
    firstDate,
    lastDate,
...
export function applyOverrides(result, overrides = [], snapshot, options = {}) {
  ...
  if (forcedMerchants.length) {
    ...
    // A merchant whose gaps match no cadence band at all falls straight out
    // of detectRecurring's per-merchant loop with nothing — the forced
    // override still needs to produce a row, or "mark recurring" silently
    // does nothing for exactly the bills irregular enough to need it.
    const stillMissing = forcedMerchants.filter((m) => !loose.series.some((s) => s.merchant === m));
    const eligible = (snapshot?.transactions ?? []).filter(
      (txn) => !txn.excluded && txn.categoryId !== 'income' && txn.categoryId !== 'transfers' && txn.amount < 0
    );
    for (const merchant of stillMissing) {
      const ordered = eligible
        .filter((t) => t.merchant === merchant)
        .sort((a, b) => (a.date < b.date ? -1 : a.date > b.date ? 1 : 0));
      const manual = buildManualSeries(merchant, ordered);
      if (manual) series.push(manual);
    }
  }
