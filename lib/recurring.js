...
export function detectRecurring(snapshot, { today = new Date().toISOString().slice(0, 10), strict = true } = {}) {
  ...
  for (const [merchant, group] of byMerchant) {
    ...
    // Convert per-occurrence amount to monthly equivalent, then annualize.
    // This ensures monthlyCost is always the per-month cost, and annualCost
    // is consistently monthlyCost * 12, matching budget/financial expectations.
    const monthlyCost = round2(profile.typical * (30.44 / cadence.days));
