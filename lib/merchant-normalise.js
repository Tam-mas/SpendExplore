...
// Payment gateway prefix, e.g. "SQ*", "SMP*", "LSP*". Pulled out as its own
// constant (rather than left inline in STRIP_PATTERNS) so normaliseMerchant
// can detect whether it actually matched, not just strip it blindly — see
// the reference-code exemption below.
const GATEWAY_PREFIX_RE = /^(?:SQ|SMP|LSP|SP|PAYPAL|PP)\s*\*\s*/i;
...
const STRIP_PATTERNS = [
  ...
  GATEWAY_PREFIX_RE,                // payment gateway prefixes
...
// over the whole string: the first content token is exempt (unless a
// payment-gateway prefix was glued directly onto it — see below), every
// later token is checked.
//
...
export function normaliseMerchant(rawDescription) {
  ...
  // A payment-gateway prefix (SQ*/SMP*/LSP*/...) is bank-terminal routing
  // metadata, not merchant text. When it's stripped, whatever token ends up
  // at index 0 is only "leading" because unrelated noise was removed in
  // front of it — it is not necessarily the genuine first content token of
  // the raw description. Record whether that happened so the reference-code
  // exemption below can be disabled in that case: otherwise "SMP*COBURG03"
  // or "SQ*WK2PZP Sydney AU" would promote a bare reference code to token 0
  // and wrongly keep it as the merchant name.
  let gatewayPrefixStripped = false;
  for (const pattern of STRIP_PATTERNS) {
    if (pattern === GATEWAY_PREFIX_RE && pattern.test(s)) gatewayPrefixStripped = true;
    s = s.replace(pattern, ' ');
  }
  ...
  // leads) — unless a gateway prefix was glued directly onto it, in which
  // case token 0 is not genuinely leading and gets the same treatment as
  // every later token.
  tokens = tokens.filter((tok, idx) =>
    (idx === 0 && !gatewayPrefixStripped) || !REFERENCE_CODE_RE.test(tok)
  );
