...
//
// `strippedFromFront: true` means the pattern removes prefix NOISE plus a
// separator (a biller number and trailing space, a marker and trailing `*`,
// ...) — so whatever ends up at token 0 afterwards is only "leading"
// because unrelated text was removed in front of it, not because it's
// genuinely the first content token. That token then loses the
// REFERENCE_CODE_RE exemption below (see `strippedFromFront` in
// normaliseMerchant).
//
// This is an explicit per-pattern property, not inferred from `^` anchoring,
// because anchoring alone doesn't tell you whether a separator was removed.
// "Open transaction details" is front-anchored but glued directly onto the
// merchant with NO separator — token 0 after stripping it IS the genuine
// first content token, so it must keep the exemption. Inferring the flag
// from `.source.startsWith('^')` got this wrong for exactly this pattern
// (every row of a real statement lost its leading brand name to "Unknown").
// Explicit and verbose beats clever and wrong: the next person adding a
// front-anchored pattern here has to consciously decide which case theirs
// is, rather than relying on structural inference that silently assumes
// "removes a separator" whenever it's actually just "removes noise".
const STRIP_PATTERNS = [
  { re: /\bValue Date:.*$/i, strippedFromFront: false },              // trailing settlement date
  { re: /\bCard\s+xx\d{4}\b/i, strippedFromFront: false },            // card identifier
  { re: /^Direct Debit\s+\d+\s*/i, strippedFromFront: true },         // direct debit prefix + biller number
  { re: /^(?:Wdl|Dep)\s+/i, strippedFromFront: true },                // withdrawal / deposit prefix
  { re: /^Open\s+transaction\s+details\s*/i, strippedFromFront: false }, // web-UI prefix, glued on with no separator
  { re: /^(?:SQ|SMP|LSP|SP|PAYPAL|PP)\s*\*\s*/i, strippedFromFront: true }, // payment gateway prefixes
  { re: /\b[A-Z]{3}\d{6,}\b/g, strippedFromFront: false },            // biller reference codes e.g. OVO4477613290
  { re: /\b(?:PTY\s+L(?:TD)?|P\/L|LIMITED|LTD|INC)\b/gi, strippedFromFront: false } // company suffixes
];
...
export function normaliseMerchant(rawDescription) {
  ...
  // Any STRIP_PATTERNS entry marked `strippedFromFront: true` removes bank
  // prefix NOISE plus a separator — a Direct Debit biller number, a Wdl/Dep
  // marker, a payment-gateway tag. Whenever one of those actually matches,
  // whatever token ends up at index 0 afterwards is only "leading" because
  // that noise was stripped in front of it — it is not necessarily the
  // genuine first content token of the raw description. A pattern marked
  // `false` (like the "Open transaction details" web-UI prefix, which is
  // glued directly onto the merchant with no separator) leaves token 0 as
  // the genuine first content token, so it must NOT set this flag.
  let strippedFromFront = false;
  for (const { re, strippedFromFront: front } of STRIP_PATTERNS) {
    if (front && re.test(s)) strippedFromFront = true;
    s = s.replace(re, ' ');
  }
